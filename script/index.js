const modalFunc = () => {
  const modal = document.querySelector('.cart-modal__overlay')
  const cartBtn = document.querySelector('#cart-button')

  // const cancelBtn = document.querySelector('#cancel-cart-button')
  // const closeBtn = document.querySelector('.cart-modal__header--close')

  const openModal = () => {
    modal.classList.add('open')
  }
  const closeModal = () => {
    modal.classList.remove('open')
  };
  cartBtn.addEventListener('click', () => {
    openModal()
  })

  modal.addEventListener('click', (event) => {
    // console.log(event.target.closest('#cancel-cart-button'));

    if (event.target.classList.contains('cart-modal__overlay')
      || event.target.closest('.cart-modal__header--close')
      || event.target.closest('#cancel-cart-button')) {
      closeModal()
    }
  })
};

const loading = (cont) => {
  cont.innerHTML = ('<p style="width:100%;text-align: center">Загрузка...</p>')
};


const restFunc = () => {
  const container = document.querySelector('#rests-container')
  const restArray = [
    {
      id: 0,
      titel: 'Пицца плюс',
      time: 50,
      rating: 4.5,
      price: 900,
      type: 'Пицца',
      image: 'image-0.jpg'
    },
    {
      id: 1,
      titel: 'Тануки',
      time: 50,
      rating: 4.5,
      price: 900,
      type: 'Пицца',
      image: 'image-1.jpg'
    },
    {
      id: 2,
      titel: 'FoodBand',
      time: 50,
      rating: 4.5,
      price: 900,
      type: 'Пицца',
      image: 'image-2.jpg'
    },
    {
      id: 3,
      titel: 'Жадина-пицца',
      time: 50,
      rating: 4.5,
      price: 900,
      type: 'Пицца',
      image: 'image-3.jpg'
    },
    {
      id: 4,
      titel: 'Точка еды',
      time: 50,
      rating: 4.5,
      price: 900,
      type: 'Пицца',
      image: 'image-4.jpg'
    },
    {
      id: 5,
      titel: 'PizzaBurger',
      time: 50,
      rating: 4.5,
      price: 900,
      type: 'Пицца',
      image: 'image-5.jpg'
    },

  ]
  const renderRests = (array) => {
    container.innerHTML = ('')
    array.forEach(card => {
      //
      container.insertAdjacentHTML("beforeend", `
            <a href="./goods.html?id=${card.id}" class="products-card">
              <div class="products-card__image">
                <img src="rests/${card.image}" alt="${card.image}">
              </div>
              <div class="products-card__description">
                <div class="products-card__description-row">
                  <h4 class="products-card__description--title">${card.titel}</h4>
                  <div class="products-card__description--bage">${card.time} мин</div>
                </div>
                <!-- /.products-card__description-row -->
                <div class="products-card__description-row">
                  <div class="products-card__description--info">
                    <div class="products-card__description--info-rating">
                      <img src="images/star.svg" alt="star">
                      ${card.rating}
                    </div>
                    <div class="products-card__description--info-price">
                      От ${card.price} ₽
                    </div>
                    <div class="products-card__description--info-group">
                      ${card.type}
                    </div>
                  </div>
                  <!-- /.products-card__description--info -->
                </div>
                <!-- /.products-card__description-row -->
              </div>
              <!-- /.products-card__description -->
            </a>
`)

    });
  };

  if (container) {
    loading(container)

    setTimeout(() => {
      renderRests(restArray)
    }, 1000)

  }

};

const goodsFunc = () => {
  const container = document.querySelector('#goods-container')
  const goodsArray = [
    {
      id: 0,
      name: 'Ролл угорь стандарт',
      desc: 'ис, угорь, соус унаги, кунжут, водоросли нори.',
      price: 250,
      image: 'image-6.jpg'
    },
    {
      id: 1,
      name: 'Калифорния лосось стандарт',
      desc: 'Рис, лосось, авокадо, огурец, майонез, икра масаго, водоросли нори.',
      price: 250,
      image: 'image-7.jpg'
    },
    {
      id: 2,
      name: 'Окинава стандарт',
      desc: 'Рис, креветка отварная, сыр сливочный, лосось, огурец свежий...',
      price: 250,
      image: 'image-8.jpg'
    },
    {
      id: 3,
      name: 'Цезарь маки хl',
      desc: 'Рис, куриная грудка копченая, икра масаго, томат, айсберг, соус цезарь...',
      price: 250,
      image: 'image-9.jpg'
    },
    {
      id: 4,
      name: 'Ясай маки стандарт 185 г',
      desc: 'Рис, помидор свежий, перец болгарский, авокадо, огурец, айсберг',
      price: 250,
      image: 'image-10.jpg'
    },
    {
      id: 5,
      name: 'Ролл с креветкой стандарт',
      desc: 'Рис, водоросли нори, креветки отварные, сыр сливочный, огурцы',
      price: 250,
      image: 'image-11.jpg'
    },
  ]
  const renderGoods = (array) => {
    container.innerHTML = ('')
    array.forEach(card => {
      //
      container.insertAdjacentHTML("beforeend", `
            <div class="products-card">
              <div class="products-card__image">
                <img src="goods/${card.image}" alt="${card.image}">
              </div>
              <div class="products-card__description">
                <div class="products-card__description-row">
                  <h5 class="products-card__description--name">${card.name}</h5>
                </div>
                <!-- /.products-card__description-row -->
                <div class="products-card__description-row">
                  <p class="products-card__description--text">${card.desc}</p>
                </div>
                <!-- /.products-card__description-row -->
                <div class="products-card__description-row">
                  <div class="products-card__description-controls">
                    <button class="btn btn-primory">
                      В корзину
                      <img src="images/shipping.svg" alt="cart" class="shipping-image">
                    </button>
                    <span class="products-card__description-controls--price">${card.price} ₽</span>
                  </div>
                </div>
                <!-- /.products-card__description-row -->
              </div>
              <!-- /.products-card__description -->
            </div>
`)

    });
  };

  if (container) {
    loading(container)

    setTimeout(() => {
      renderGoods(goodsArray)
    }, 1000)

  }


};

restFunc()
goodsFunc()
modalFunc()